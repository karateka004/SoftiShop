...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // Загрузка из localStorage после монтирования (SSR-безопасно: сервер и первый
  // клиентский рендер дают [] — без hydration mismatch; затем синхронизируемся).
  useEffect(() => {
    try {
      const raw = localStorage.getItem(STORAGE_KEY);
      // eslint-disable-next-line react-hooks/set-state-in-effect -- разовая гидрация из localStorage; пустой старт обязателен для совпадения SSR-рендера
